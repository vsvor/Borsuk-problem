#!/usr/bin/env python3
"""Verify the four convex hulls in r_dod_4_0966.txt against the intended polyhedron.

Usage:
    python verify_rd_cover.py r_dod_4_0966.txt --json r_dod_4_0966_report.json
...
def load_input(path: Path):
    ...
    if n != 43 or not np.all(np.isfinite(points)):
        raise ValueError('Expected 43 finite points.')
    for g in groups:
...
class HullDistance:
    ...
    def __init__(self, points, hull):
        ...
        self.den = np.einsum('ij,ij->i', normals, normals)
        self.cross_normals = normals
    ...
    def __call__(self, x, with_point=False):
        ...
        # Cross-product barycentric coordinates avoid subtracting nearly equal
        # products for skinny triangles.
        ub = np.einsum('ij,ij->i', np.cross(pp-self.a, self.f), self.cross_normals) / self.den
        uc = np.einsum('ij,ij->i', np.cross(self.e, pp-self.a), self.cross_normals) / self.den
...
def boundary_cover(X, Y, groups, N):
    ...
    for face_id, normal in enumerate(N):
        ...
        ids = np.flatnonzero(np.abs(X[:23] @ normal - .5) < 1e-12)
        p = Y[ids] @ basis
...
def verify(path: Path, tolerance: float):
    ...
    target_hull = ConvexHull(X[:23])
    with localcontext() as ctx:
        ctx.prec = 60
        target_volume_decimal = Decimal(7) / 2 - Decimal(2) * Decimal(2).sqrt()
    ...
    result: dict[str, Any] = {
        ...
        'number_of_points': len(X), 'groups': groups,
        'indexing': 'zero-based', 'stored_value': stored,
        'target': {'description': 'x,y,z <= 1/2; |x_i|+|x_j| <= 1/sqrt(2)',
                   'face_count': 15, 'vertex_count': 23,
                   'volume': target_volume, 'volume_formula': '7/2 - 2*sqrt(2)',
                   'volume_decimal': str(target_volume_decimal),
                   'volume_from_first_23_points': float(target_hull.volume)},
    ...
    result['volumes'] = {
        ...
        'symmetric_difference': target_volume + raw_union - 2*inside_union,
    ...
    result['largest_hull_diameter'] = max(h['diameter'] for h in result['hulls'])
    result['diameter_exceedances'] = [
        {'hull': i+1, 'pair': [a,b], 'distance': float(np.linalg.norm(X[a]-X[b]))}
        for i,g in enumerate(groups) for a,b in itertools.combinations(g,2)
        if np.linalg.norm(X[a]-X[b]) > stored+1e-12
    ]
    return result
...
def main():
    ...
    parser.add_argument('--json', type=Path, default=Path('r_dod_4_0966_report.json'))
